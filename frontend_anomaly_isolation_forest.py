...
# Function to load column-specific Isolation Forest models
def load_column_model(node_index, column_name):
    model_path = os.path.join(MODEL_DIR, f"Isolation_Forest_Model_Node_{node_index}_Column_{column_name}.pkl")
...
def app(node):
    ...
    st.markdown("Analyze anomalies for each feature using Isolation Forest models trained for each column.")
    ...
    # Display accuracy and data below
    display_node_accuracy(node_index)

    # Detect anomalies for visualization (optional if not refreshed)
    for column in data.select_dtypes(include=[np.number]).columns:
        model = load_column_model(node_index, column)
        if model is None:
            continue
        ...
        if anomaly_count > 0:
            ...
            st.markdown(
                f"""
                <div style="border: 2px solid; padding: 10px; margin-top: 30px; border-radius: 10px; text-align: center;">
